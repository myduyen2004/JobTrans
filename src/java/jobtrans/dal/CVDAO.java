...
public class CVDAO {
    ...
    public List<String> getAllSchoolNames() throws Exception {
        ...
        try (Connection conn = dbConnection.openConnection(); PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
    ...
    public boolean deleteSkillByCvId(int cvId) throws Exception {
        ...
        try (Connection conn = dbConnection.openConnection(); PreparedStatement stmt = conn.prepareStatement(query)) {
    ...
    public CV getAppliedCVByUserIdAndJobId(int userId, int jobId) {
        CV cv = null;
        String sql = "SELECT * FROM CV "
                + "INNER JOIN JobGreetings ON CV.cv_id = JobGreetings.cv_id "
                + "WHERE JobGreetings.user_id = ? AND JobGreetings.job_id = ?";
        ...
        try (PreparedStatement statement = dbConnection.openConnection().prepareStatement(sql)) {
            ...
            if (resultSet.next()) {  // Chỉ lấy CV đầu tiên
                int cvId = resultSet.getInt("cv_id");
                String title = resultSet.getString("job_position");
                String summary = resultSet.getString("summary");
                Date createdAt = resultSet.getDate("created_at");

                cv = new CV(cvId, title, summary, createdAt, userId);
                cv.setListEdu(getEducationByCVId(cvId));
                cv.setListCertification(getListCertificationByCvId(cvId));
                cv.setListExperience(getListExperienceByCvId(cvId));
                cv.setListSkill(getListSkillByCvId(cvId));
