...
<html lang="en">
    ...
    <body>
        ...
        <%
            String check = (String) request.getAttribute("prepay-check");
            if(check.equals("Đã trả trước") ){
        %>
        ...
        <div class="payment-info">
            ...
            <strong>
                <fmt:formatNumber value="${prepayMoney}" type="currency" pattern="#,##0" currencySymbol="₫" groupingUsed="true" />
            </strong>
            <button class="btn-tra-truoc" onclick="window.location.href = './myjob?action=prepay-employer&jobId=${job.jobId}&prepayMoney=${prepayMoney}';">Trả trước</button>
        </div>
        ...
        <h2>Trả trước cho bài đăng</h2>
        <%
            String check = (String) request.getAttribute("deposit-check");
            if(check.equals("Đã cọc") ){
        %>
        <div class="payment-info">
            <span>Bạn đã đặt cọc cho công việc này</span>
            <button class="btn-tra-truoc" onclick="window.location.href = './profile?action=wallet';">Xem lịch sử giao dịch</button>
        ...
        <div class="payment-info">
            <span>Số tiền cần trả: </span>
            <strong>
                <fmt:formatNumber value="${depositMoney}" type="currency" pattern="#,##0" currencySymbol="₫" groupingUsed="true" />
            </strong>
            <button class="btn-tra-truoc" onclick="window.location.href = './myjob?action=deposit-seeker&jobId=${job.jobId}&depositMoney=${depositMoney}';">Trả trước</button>
        </div>
        <div class="payment-note">
            <p class="note-green">Số tiền được trả sẽ được hệ thống tạm thời giữ, sau khi công việc xác nhận hoàn thành sẽ được thanh lý bởi hệ thống</p>
            <p class="note-red">Trong trường hợp công việc bị gián đoạn, hệ thống sẽ hoàn trả</p>
